    ...
    
{
    ...
    if(pSession->DefinesParameter("N_slices"))
    {
        ...
        if(m_slices>1)
        {
            ...
            int step = fmod(time,m_period)/m_session->GetParameter("TimeStep");
            if (adj)
                step = m_slices - step;
            ...
            cout << buffer << " " << time << " " << " getfldbase\n";

            ImportFldBase(buffer,pFields,1);
        ...
        int                                          pSlice)
{
    ...
    //for (int j = 0; j < m_baseflow[0].size(); ++j)   
    //{
    //    cout << "basefl " << j << " " << m_baseflow[0][j] << "\n";
    //}
